(ns marketentry.governor-contract-test
  ...
  This is the SDN actor's SIX-check governor (see `marketentry.governor`
  ...
  distinct SDN tax-ID scheme. `registration-missing` is the SDN analog
  of AGO's `ao-entity-missing` flagship check / STP's
  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Khartoum Trading Co."}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Khartoum Trading Co." (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest registration-missing-is-held-and-unoverridable
  (testing "missing SDN commercial/corporate registration -> HARD hold (SDN flagship check)"
    (let [[db actor] (fresh)
      ...
      (is (some #{:registration-missing} (-> (store/ledger db) last :basis)))
