(ns marketentry.marketentryllm
  ...
  the SDN public-sector market-entry compliance actor.
...
(defn- propose-submit
  ...
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向け提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "has-sdn-registration?=" (:has-sdn-registration? e)
     ...
     :confidence (if (and e
                          (or (not (:requires-sdn-registration? e))
                              (:has-sdn-registration? e)))
                   0.9 0.3)}))
